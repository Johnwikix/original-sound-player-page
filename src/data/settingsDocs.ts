    ...
    items: [
      {
        ...
        type: 'text',
        description: '需要 Windows 10 1809 或更高版本（包括 Windows 11）。建议使用 Windows 11 以获得完整的 Mica / Acrylic 视觉效果。',
        descriptionEn:
    ...
    items: [
      {
        ...
        type: 'select',
        description: '选择音频输出设备。下拉列表会枚举系统中所有可用的输出端（如扬声器、耳机、USB DAC）。',
        descriptionEn: 'Select the audio output device. The dropdown enumerates all available outputs in the system (e.g., speakers, headphones, USB DAC).',
        options: ['WASAPI 独占(推送)', 'WASAPI 独占(事件)', 'WASAPI 共享', 'WaveOut', 'DirectSound', 'ASIO'],
        optionsEn: ['WASAPI Exclusive (Push)', 'WASAPI Exclusive (Event)', 'WASAPI Shared', 'WaveOut', 'DirectSound', 'ASIO']
      },
      {
        name: '缓冲区大小',
        nameEn: 'Buffer Size',
        type: 'number',
        description: '音频缓冲区大小（单位：毫秒），范围 100-900。数值越小延迟越低，但可能引起爆音；数值越大越稳定。',
        descriptionEn: 'Audio buffer size in milliseconds, range 100-900. Smaller values reduce latency but may cause audio glitches; larger values are more stable.',
      ...
      },
      {
        ...
        descriptionEn: 'The sample rate used when converting DSD audio to PCM. Only takes effect when DSD mode is "DoP".',
        options: ['44.1 kHz', '48 kHz', '88.2 kHz', '96 kHz', '176.4 kHz', '192 kHz', '352.8 kHz', '384 kHz'],
        optionsEn: ['44.1 kHz', '48 kHz', '88.2 kHz', '96 kHz', '176.4 kHz', '192 kHz', '352.8 kHz', '384 kHz']
      },
      {
        name: 'DSD 增益',
        nameEn: 'DSD Gain',
        type: 'number',
        description: 'DSD 信号到 PCM 的增益补偿（单位：dB），可降低 DSD→PCM 转换后的音量损失。仅 DoP 模式生效。',
        descriptionEn: 'Gain compensation (in dB) when converting DSD to PCM, reducing the volume loss after conversion. Only effective in DoP mode.',
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        descriptionEn: 'Release memory used by cached inactive tracks when the window is minimized or hidden.'
      },
      {
        name: '播放后释放',
        ...
        descriptionEn: 'Automatically release a track\'s decoded buffer from memory after playback completes.'
      },
      {
        name: '立即释放',
        nameEn: 'Trim Now',
        ...
        descriptionEn: 'Manually trigger a memory cleanup to immediately release all inactive track caches.'
      },
      {
        name: '重置窗口位置',
        ...
        descriptionEn: 'Restore the application window to its default position and size, clearing the saved window bounds.'
      }
    ]
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        descriptionEn: 'Show or hide the now-playing detail page (with cover and full-screen lyrics).'
      },
      {
        name: '返回',
        nameEn: 'Back',
      ...
      },
      {
        ...
        descriptionEn: 'Bring the application window to the foreground when it is in the background.'
      },
      {
        name: '全屏切换',
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        type: 'text',
        description: '封面图片的本地缓存目录。点击可打开所在文件夹，或修改到其他位置。修改后需重启应用。',
        descriptionEn:
      ...
      },
      {
        ...
        options: ['Fluid', 'PS3XMB', 'GradientFlow', 'WavyBackground', 'ChromaticResonance', 'LiquidFlow'],
        optionsEn: ['Fluid', 'PS3XMB', 'GradientFlow', 'WavyBackground', 'ChromaticResonance', 'LiquidFlow']
      },
      {
        name: '光波',
        nameEn: 'Light Wave',
        ...
        descriptionEn: 'Overlay a light wave effect on top of the shader background. Only available when shader background is enabled.'
      },
      {
        name: '雾',
        nameEn: 'Fog',
      ...
      },
      {
        ...
        type: 'select',
        description: '从封面提取主色调时使用的算法：K-Means++（速度较慢但准确）或 OctTree（速度更快）。',
        descriptionEn:
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        descriptionEn: 'Font family used for displaying lyrics. Choose from installed system fonts.'
      },
      {
        name: '歌词字号',
      ...
      },
      {
        ...
        descriptionEn: 'Blur amount for unplayed lyric lines. Higher values blur the unplayed lines more.',
      ...
      },
      {
        ...
        descriptionEn: 'Opacity of unplayed lyric lines. Smaller values make them more transparent.',
      ...
      },
      {
        ...
        descriptionEn: 'Minimum duration (ms) to be classified as a long syllable for advanced lyric animation.',
      ...
      },
      {
        ...
        descriptionEn: 'Target frame rate for advanced lyric animation. Affects smoothness and power consumption.',
      ...
      },
      {
        ...
        descriptionEn: 'Enable a custom color for played lyric lines. Requires the "Advanced Lyrics Effect" to be enabled first.'
      }
    ]
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        descriptionEn: 'Opacity of the custom acrylic backdrop (only available when backdrop type is "Custom Acrylic").',
        range: { min: 0, max: 100, step: 1, unit: '%' }
      },
      {
        name: '自定义颜色',
        ...
        descriptionEn: 'Custom color for the acrylic backdrop (only available when backdrop type is "Custom Acrylic").'
      },
      {
        name: '提取主色',
        nameEn: 'Use Image Dominant Theme',
        type: 'toggle',
        description: '根据当前播放曲目的封面图片自动提取主色调，并应用到界面元素。仅当流体背景启用时可用。',
        descriptionEn:
